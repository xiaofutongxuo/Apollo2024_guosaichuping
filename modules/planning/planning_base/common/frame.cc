...
FrameHistory::FrameHistory() : IndexedQueue<uint32_t, Frame>(FLAGS_max_frame_history_num) {}

Frame::Frame(uint32_t sequence_num) :
        sequence_num_(sequence_num), monitor_logger_buffer_(common::monitor::MonitorMessageItem::PLANNING) {}
...
bool Frame::Rerouting(PlanningContext *planning_context) {
    ...
    // // 如果导航模式启用,则不能rerouting
    // if (FLAGS_use_navigation_mode) {
    //     AERROR << "Rerouting not supported in navigation mode";
    //     return false;
    // }

    // // 如果没有先前的routing信息,则不能rerouting
    // if (local_view_.planning_command == nullptr) {
    //     AERROR << "No previous routing available";
    //     return false;
    // }

    // // 如果没有有效的高精地图,则不能rerouting
    // if (!hdmap_) {
    //     AERROR << "Invalid HD Map.";
    //     return false;
    // }

    // // 获取先前的routing_request
    ...
    // // 在高精地图中查找最近的车道
    // if (hdmap_->GetNearestLaneWithHeading(point, 5.0, vehicle_state_.heading(), M_PI / 3.0, &lane, &s, &l) != 0) {
    //     AERROR << "Failed to find nearest lane from map at position: " << point.DebugString()
    //            << ", heading:" << vehicle_state_.heading();
    //     return false;
    // }

    // auto point_temp = request.waypoint(2);
    // // AINFO<<"point_temp的信息为:         "<<point_temp.DebugString();
    ...
    // end_point->set_id(point_temp.id());
    // end_point->set_s(point_temp.s());
    // end_point->mutable_pose()->CopyFrom(point_temp.pose());

    // AINFO << "waypoint:  -------" << request.DebugString();
...
bool Frame::CreateReferenceLineInfo(
    ...
    for (auto iter = reference_line_info_.begin(); iter != reference_line_info_.end();) {
...
const Obstacle *Frame::CreateStaticObstacle(
    ...
    common::math::Box2d obstacle_box{
            common::math::LineSegment2d(obstacle_start_xy, obstacle_end_xy), left_lane_width + right_lane_width};
...
Status Frame::InitForOpenSpace(const common::VehicleStateProvider *vehicle_state_provider, const EgoInfo *ego_info) {
    return InitFrameData(vehicle_state_provider, ego_info);
}

Status Frame::InitFrameData(const common::VehicleStateProvider *vehicle_state_provider, const EgoInfo *ego_info) {
    ...
    if (planning_start_point_.v() < 1e-3) {
        ...
        if (collision_obstacle != nullptr) {
            const std::string msg = absl::StrCat("Found collision with obstacle: ", collision_obstacle->Id());
            AERROR << msg;
...
void Frame::RecordInputDebug(planning_internal::Debug *debug) {
    ...
    planning_debug_data->mutable_prediction_header()->CopyFrom(local_view_.prediction_obstacles->header());
    /*
...
void Frame::AlignPredictionTime(const double planning_start_time, PredictionObstacles *prediction_obstacles) {
    ...
    for (auto &obstacle : *prediction_obstacles->mutable_prediction_obstacle()) {
        for (auto &trajectory : *obstacle.mutable_trajectory()) {
            for (auto &point : *trajectory.mutable_trajectory_point()) {
                point.set_relative_time(prediction_header_time + point.relative_time() - planning_start_time);
            }
            if (!trajectory.trajectory_point().empty() && trajectory.trajectory_point().begin()->relative_time() < 0) {
