...
bool ReferenceLineInfo::Init(const std::vector<const Obstacle*>& obstacles, double target_speed) {
    ...
    if (adc_sl_boundary_.end_s() < 0 || adc_sl_boundary_.start_s() > reference_line_.Length()) {
        AWARN << "Vehicle SL " << adc_sl_boundary_.ShortDebugString() << " is not on reference line:[0, "
              << reference_line_.Length() << "]";
    }
    static constexpr double kOutOfReferenceLineL = 14.0;  // in meters
    if (adc_sl_boundary_.start_l() > kOutOfReferenceLineL || adc_sl_boundary_.end_l() < -kOutOfReferenceLineL) {
    ...
    for (const auto& speed_bump : map_path.speed_bump_overlaps()) {
        ...
        reference_line_.AddSpeedLimit(speed_bump.start_s - 1.0, speed_bump.end_s + 1.0, FLAGS_speed_bump_speed_limit);
    }
...
double ReferenceLineInfo::GetBaseCruiseSpeed() const {
    return base_cruise_speed_ > 0.0 ? base_cruise_speed_ : FLAGS_default_cruise_speed;
}
...
void ReferenceLineInfo::InitFirstOverlaps() {
    ...
    if (!first_encounter_overlaps_.empty()) {
        std::sort(
                first_encounter_overlaps_.begin(),
...
void ReferenceLineInfo::SetJunctionRightOfWay(const double junction_s, const bool is_protected) const {
...
Obstacle* ReferenceLineInfo::AddObstacle(const Obstacle* obstacle) {
    ...
    if (IsIrrelevantObstacle(*mutable_obstacle)) {
        ...
        ADEBUG << "reference line st boundary: t[" << mutable_obstacle->reference_line_st_boundary().min_t() << ", "
...
bool ReferenceLineInfo::AddObstacles(const std::vector<const Obstacle*>& obstacles) {
    if (FLAGS_use_multi_thread_to_add_obstacles) {
        ...
        for (const auto* obstacle : obstacles) {
            // -----------------add-----------------
            if (obstacle->Id() == "9073" || obstacle->Id() == "9073_0") {
                continue;
            }
            // -------------------------------------
...
bool ReferenceLineInfo::IsIrrelevantObstacle(const Obstacle& obstacle) {
    ...
    if (is_on_reference_line_ && !IsChangeLanePath()
        && adc_sl_boundary_.end_s() - obstacle_boundary.end_s() > FLAGS_obstacle_lon_ignore_buffer
...
bool ReferenceLineInfo::AdjustTrajectoryWhichStartsFromCurrentPos(
    ...
    for (size_t i = 0; i < trajectory.size(); ++i) {
        ...
        const double tracking_heading = std::atan2(cur_point_y - start_point_y, cur_point_x - start_point_x);
    ...
    // cause.
    if (cut_trajectory.size() > 1 && cut_trajectory.front().relative_time() >= cut_trajectory[1].relative_time()) {
...
std::string ReferenceLineInfo::PathSpeedDebugString() const {
    return absl::StrCat("path_data:", path_data_.DebugString(), "speed_data:", speed_data_.DebugString());
}

void ReferenceLineInfo::SetTurnSignalBasedOnLaneTurnType(common::VehicleSignal* vehicle_signal) const {
    CHECK_NOTNULL(vehicle_signal);
    if (vehicle_signal->has_turn_signal() && vehicle_signal->turn_signal() != VehicleSignal::TURN_NONE) {
        return;
    }
    ...
    for (const auto& seg : Lanes()) {
        ...
        if (turn == hdmap::Lane::LEFT_TURN) {
            ...
            auto middle_xy = PointFactory::ToVec2d(seg.lane->GetSmoothPoint((seg.start_s + seg.end_s) / 2.0));
...
double ReferenceLineInfo::SDistanceToDestination() const {
    ...
    const double stop_s
            = dest_ptr->PerceptionSLBoundary().start_s() + dest_ptr->LongitudinalDecision().stop().distance_s();
    return stop_s - adc_sl_boundary_.end_s();
}

void ReferenceLineInfo::ExportDecision(DecisionResult* decision_result, PlanningContext* planning_context) const {
...
void ReferenceLineInfo::MakeDecision(DecisionResult* decision_result, PlanningContext* planning_context) const {
...
void ReferenceLineInfo::MakeMainMissionCompleteDecision(
    ...
    if (main_stop.reason_code() != STOP_REASON_DESTINATION && main_stop.reason_code() != STOP_REASON_PULL_OVER) {
        return;
    }
    const double distance_destination = SDistanceToDestination();
    ...
    if (ReachedDestination()) {
        planning_context->mutable_planning_status()->mutable_destination()->set_has_passed_destination(true);
    } else {
...
int ReferenceLineInfo::MakeMainStopDecision(DecisionResult* decision_result) const {
    ...
    for (const auto* obstacle : path_decision_.obstacles().Items()) {
        ...
        if (stop_line_s < 0 || stop_line_s > reference_line_.Length()) {
            AERROR << "Ignore object:" << obstacle->Id() << " fence route_s[" << stop_line_s << "] not in range[0, "
                   << reference_line_.Length() << "]";
            continue;
        }
...
void ReferenceLineInfo::ExportEngageAdvice(EngageAdvice* engage_advice, PlanningContext* planning_context) const {
...
int ReferenceLineInfo::GetJunction(const double s, hdmap::PathOverlap* junction_overlap) const {
    ...
    const std::vector<hdmap::PathOverlap>& junction_overlaps = reference_line_.map_path().junction_overlaps();
...
std::vector<common::SLPoint> ReferenceLineInfo::GetAllStopDecisionSLPoint() const {
    ...
    // sort by s
    if (!result.empty()) {
        std::sort(result.begin(), result.end(), [](const common::SLPoint& a, const common::SLPoint& b) {
            return a.s() < b.s();
        });
    }
