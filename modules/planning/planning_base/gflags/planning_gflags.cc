...
DEFINE_bool(enable_force_pull_over_open_space_parking_test, false, "enable force_pull_over_open_space_parking_test");
...
DEFINE_string(rtk_trajectory_filename, "modules/planning/planning_base/data/garage.csv", "Loop rate for planning node");
...
DEFINE_double(rtk_trajectory_resolution, 0.01, "The time resolution of output trajectory for rtk planner.");
...
DEFINE_double(
        smoothed_reference_line_max_diff,
        5.0,
...
// Decision Part
DEFINE_double(static_obstacle_nudge_l_buffer, 0.3, "minimum l-distance to nudge a static obstacle (meters)");
DEFINE_double(nonstatic_obstacle_nudge_l_buffer, 0.4, "minimum l-distance to nudge a non-static obstacle (meters)");
DEFINE_double(
...
DEFINE_double(passed_destination_threshold, 0.05, "check adc whether has passed destination");
...
// intersection.
DEFINE_double(turn_signal_distance, 100.00, "In meters. If there is a turn within this distance, use turn signal");

DEFINE_int32(trajectory_point_num_for_debug, 10, "number of output trajectory points for debugging");

// QpSt optimizer
DEFINE_double(slowdown_profile_deceleration, -4.0, "The deceleration to generate slowdown profile. unit: m/s^2.");

DEFINE_double(speed_fallback_distance, 3.0, "Distance to speed fallback. unit: m.");
/// thread pool
...
DEFINE_double(speed_lon_decision_horizon, 200.0, "Longitudinal horizon for speed decision making (meter)");
...
DEFINE_double(lon_collision_buffer, 2.0, "The longitudinal buffer to keep distance to other vehicles");
DEFINE_double(lat_collision_buffer, 0.1, "The lateral buffer to keep distance to other vehicles");
DEFINE_uint64(num_sample_follow_per_timestamp, 3, "The number of sample points for each timestamp to follow");
...
DEFINE_double(lon_collision_cost_std, 0.5, "The standard deviation of longitudinal collision cost function");
...
DEFINE_bool(lateral_optimization, true, "whether using optimization for lateral trajectory generation");
DEFINE_double(
        weight_lateral_offset,
        1.0,
        "weight for lateral offset "
        "in lateral trajectory optimization");
DEFINE_double(
        weight_lateral_derivative,
        500.0,
        "weight for lateral derivative "
        "in lateral trajectory optimization");
DEFINE_double(
        weight_lateral_second_order_derivative,
        1000.0,
        "weight for lateral second order derivative "
        "in lateral trajectory optimization");
DEFINE_double(
        weight_lateral_third_order_derivative,
        1000.0,
...
DEFINE_double(lateral_third_order_derivative_max, 0.1, "the maximal allowance for lateral third order derivative");
DEFINE_double(max_s_lateral_optimization, 60.0, "The maximal s for lateral optimization.");
DEFINE_double(default_delta_s_lateral_optimization, 1.0, "The default delta s for lateral optimization.");
...
DEFINE_double(default_city_road_speed_limit, 15.67, "default speed limit (m/s) for city road. 35 mph.");
...
DEFINE_string(
        planner_open_space_config_filename,
        "/apollo/modules/planning/planning_component/conf/"
...
DEFINE_bool(enable_smoother_failsafe, false, "whether to use warm start result as final output when smoother fails");
...
DEFINE_bool(enable_print_curve, false, "True to enable print curve info into log");
...
DEFINE_bool(enable_planning_smoother, false, "True to enable planning smoother among different planning cycles.");
DEFINE_double(
        smoother_stop_distance,
        10.0,
...
DEFINE_uint64(trajectory_stitching_preserved_length, 20, "preserved points number in trajectory stitching");

DEFINE_double(side_pass_driving_width_l_buffer, 0.1, "(unit: meter) for side pass driving width l buffer");
...
DEFINE_bool(planning_offline_learning, false, "offline learning. read record files and dump learning_data");
DEFINE_string(
        planning_data_dir,
        "/apollo/modules/planning/planning_base/data/",
...
DEFINE_int32(learning_data_frame_num_per_file, 100, "number of learning_data_frame to write out in one data file.");
DEFINE_string(
...
DEFINE_double(path_bounds_decider_resolution, 0.5, "The distance along s direction.");
...
DEFINE_double(replan_longitudinal_distance_threshold, 2.5, "The longitudinal distance threshold of replan");
DEFINE_double(replan_time_threshold, 7.0, "The time threshold of replan");
DEFINE_double(trajectory_check_collision_time_step, 1.0, "checks collision time step for trajectory");

DEFINE_double(
        obstacle_pass_check_distance,
        3.0,
        "at the distance, the obstacle goes around left or right"
        "consider ego and obstacle position");

DEFINE_bool(
        speed_optimize_fail_relax_velocity_constraint,
        true,
        "When the speed optimization fails,"
        "relax the speed upper bound constraint and try to optimize again");

DEFINE_bool(
        check_collision_freespace_obstacle_vertices,
        false,
...
DEFINE_double(
        referfece_line_max_forward_heading_diff,
        3.1415926536 / 2,
        "max angle difference between the forward reference line and ego heading");

DEFINE_double(
        referfece_line_max_backward_heading_diff,
        3.1415926536 / 6,
...
// rerouting
DEFINE_bool(rerouting_count, true, "enable bare_intersection scenarios in planning");
