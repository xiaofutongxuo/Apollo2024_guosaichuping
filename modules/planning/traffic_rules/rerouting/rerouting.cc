...
bool Rerouting::ChangeLaneFailRerouting() {
    // --------------------------------adjust--------------------------------
    const auto vehicle_state = injector_->vehicle_state();  // 车辆状态
    AINFO << "车辆的x坐标为:     " << vehicle_state->x();

    double delta = sqrt(pow((vehicle_state->x() - 423866), 2) + pow((vehicle_state->y() - 4438120), 2));
    AINFO << "delta:---------" << delta;

    if (delta < 5 && FLAGS_rerouting_count) {
        AINFO << "开始重新路由--------------";
        frame_->Rerouting(injector_->planning_context());
        FLAGS_rerouting_count = false;
        return true;
    }
    // ----------------------------------------------------------------------
    ...
    if (sl_point.s() > adc_s + prepare_distance) {
        ADEBUG << "No need rerouting now because still can drive for time: " << prepare_rerouting_time << " seconds";
        return true;
    }
    ...
    if (rerouting->has_last_rerouting_time()
        && (current_time - rerouting->last_rerouting_time() < config_.cooldown_time())) {
