...
bool SpeedDecider::Init(
    ...
    for (const auto& follow_function : config_.follow_distance_scheduler().follow_distance()) {
        follow_distance_function_.emplace_back(std::make_pair(follow_function.speed(), follow_function.slope()));
    }
...
bool SpeedDecider::IsFollowTooClose(const Obstacle& obstacle) const {
    ...
    auto* planning_status = injector_->planning_context()->mutable_planning_status()->mutable_change_lane();
        ...
        case CROSS:
            if (mutable_obstacle->IsBlockingObstacle()) {
                ...
                const std::string msg
                        = absl::StrCat("Failed to find a solution for crossing obstacle: ", mutable_obstacle->Id());
                AERROR << msg;
                return Status(ErrorCode::PLANNING_ERROR, msg);
            }
            break;
...
bool SpeedDecider::CheckStopForPedestrian(const Obstacle& obstacle) const {
    ...
    if (obstacle.path_st_boundary().min_s() < kSDistanceStartTimer) {
        const auto obstacle_speed = std::hypot(perception_obstacle.velocity().x(), perception_obstacle.velocity().y());
...
double SpeedDecider::EstimateProperFollowGap(const double& adc_speed) const {
    ...
    AINFO << "stop follow distance: " << config_.stop_follow_distance();
