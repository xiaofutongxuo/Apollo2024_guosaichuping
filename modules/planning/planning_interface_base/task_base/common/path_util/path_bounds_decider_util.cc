...
bool CompareLeftBound(const std::pair<std::string, double>& lhs, const std::pair<std::string, double>& rhs) {
    if (lhs.first == rhs.first) {
        return false;
    }
    return lhs.second < rhs.second;
}
bool CompareRightBound(const std::pair<std::string, double>& lhs, const std::pair<std::string, double>& rhs) {
    if (lhs.first == rhs.first) {
        return false;
    }
    return lhs.second > rhs.second;
...
bool PathBoundsDeciderUtil::GetBoundaryFromStaticObstacles(
    ...
    std::multiset<std::pair<std::string, double>, decltype(CompareRightBound)*> right_bounds(CompareRightBound);
    std::multiset<std::pair<std::string, double>, decltype(CompareLeftBound)*> left_bounds(CompareLeftBound);
        ...
        if (obs_idx < sorted_obstacles.size() && std::get<1>(sorted_obstacles[obs_idx]) < curr_s) {
            while (obs_idx < sorted_obstacles.size() && std::get<1>(sorted_obstacles[obs_idx]) < curr_s) {
                ...
                ADEBUG << "id[" << curr_obstacle_id << "] s[" << curr_obstacle_s << "] curr_obstacle_l_min["
                       << curr_obstacle_l_min << "] curr_obstacle_l_max[" << curr_obstacle_l_max << "] center_line["
                       << center_line << "]";
                AINFO << "id[" << curr_obstacle_id << "] s[" << curr_obstacle_s << "] curr_obstacle_l_min["
                      << curr_obstacle_l_min << "] curr_obstacle_l_max[" << curr_obstacle_l_max << "] center_line["
                      << center_line << "]";
    ...
    AINFO << "blocking_obstacle_id" << *blocking_obstacle_id << "," << path_blocked_idx;
...
std::vector<ObstacleEdge> PathBoundsDeciderUtil::SortObstaclesForSweepLine(
    ...
    for (const auto* obstacle : indexed_obstacles.Items()) {
        ...
        sorted_obstacles.emplace_back(
                1,
                obstacle_sl.start_s() - FLAGS_obstacle_lon_start_buffer,
                obstacle_sl.start_l() - FLAGS_obstacle_lat_buffer,
                obstacle_sl.end_l() + FLAGS_obstacle_lat_buffer,
                obstacle->Id());
        sorted_obstacles.emplace_back(
                0,
                obstacle_sl.end_s() + FLAGS_obstacle_lon_end_buffer,
                obstacle_sl.start_l() - FLAGS_obstacle_lat_buffer,
                obstacle_sl.end_l() + FLAGS_obstacle_lat_buffer,
                obstacle->Id());
    }
    ...
    // Sort.
    std::sort(sorted_obstacles.begin(), sorted_obstacles.end(), [](const ObstacleEdge& lhs, const ObstacleEdge& rhs) {
...
bool PathBoundsDeciderUtil::IsWithinPathDeciderScopeObstacle(const Obstacle& obstacle) {
    ...
    //     // if (obstacle.Id() == "7366") {
    //     //     ADEBUG << "obstacle is 7366";
    //     //     AINFO << "obstacle is 7366";
    //     //     return true;
    //     // } else {
    //     //     ADEBUG << "static obstacle, need to surpass";
    //     //     AINFO << "static obstacle, need to surpass";
    //     //     return false;
    //     // }
    //     return false;
    // }
    if (obstacle.IsStatic()) {
        ADEBUG << "static obstacle";
        AINFO << "static obstacle";
        return false;
    }
    if (obstacle.Id() == "7366" or obstacle.Id() == "7366_0") {
        return false;
    }
...
bool PathBoundsDeciderUtil::ComputeSLBoundaryIntersection(
    ...
    *ptr_l_max = -std::numeric_limits<double>::max();
    ...
    for (auto i = 0; i < sl_boundary.boundary_point_size(); ++i) {
        ...
        if (common::util::WithinBound<double>(std::fmin(p0.s(), p1.s()), std::fmax(p0.s(), p1.s()), s)) {
